...
public class RecentSubmissionsDaoImpl implements RecentSubmissionsDao{
	...
	public List<RecentSubmission> getRecentSubmissions(int id){
		return template.query("SELECT p.id as id,c.name as submission_course_name, p.title as submission_title, p.postDate as submission_date,c.meetingDay as submission_time_class_meets,p.note as submission_notes,a.link as submission_link,pf.active AS submission_flag,c.id as courseID,c.description as description FROM post p JOIN course c on p.courseID = c.id join attachments a on p.id = a.postID JOIN post_flag pf on p.id = pf.postID where userID = " + id + " order by postDate desc",new RowMapper<RecentSubmission>(){    
